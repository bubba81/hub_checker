...
# ── Keys tab API ──────────────────────────────────────────────────────────────
@app.route("/api/pins/mine", methods=["GET"])
@require_scanner_user
def api_pins_mine():
    """GET /api/pins/mine
    Returns only the pins generated by the currently logged-in user.
    Filters by matching the discord_user_id prefix in the label field
    (format: "discord_user_id" or "discord_user_id|human label").
    """
    uid = session.get("user_id", "")
    now = _now()
    with get_conn() as conn:
    ...
    for r in rows:
        d = dict(r)
        # Strip uid prefix from label for display
        raw_label = d.get("label", "")
        if "|" in raw_label:
            d["display_label"] = raw_label.split("|", 1)[1].strip()
        elif raw_label == uid:
            d["display_label"] = ""
        else:
            d["display_label"] = raw_label
        d["expired"] = bool(d.get("expires_at") and d["expires_at"] < now and not d["used"])
        result.append(d)
    return jsonify(result)
